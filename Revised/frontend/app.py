...
with st.form("optimization_form"):
    ...
    if submitted:
        ...
        if response.status_code == 200:
            ...
            # Debug output
            st.write("📦 Full response JSON:", response.json())

            if "optimized_beds" in result and "optimized_staff" in result:
                st.success("✅ Optimization Completed")
                st.write(f"Optimized Resource Allocation → Beds: {result['optimized_beds']}, Staff: {result['optimized_staff']}")
                st.write(f"Solver Status: {result['status']}")
            else:
                st.error("⚠️ Optimization result missing keys.")
        else:
